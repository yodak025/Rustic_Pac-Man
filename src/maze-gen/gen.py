...
class CellConnectionsGenerator:
    ...
    def _l_generator(self) -> bool:
        if (self.firts_cell.x > 0
            # -- Si hay una celda conectada por la derecha
            and self.firts_cell.is_connected_at[RIGHT]
            # -- que tiene una celda a su derecha
            and self.firts_cell.next[RIGHT]
            # -- y existe otra celda a la derecha de esta
            and self.firts_cell.next[RIGHT].next[RIGHT]
            and self.long_figures <= self.MAX_LONG_FIGURES  # -- y si se cumplen las condiciones
                # ! Numero magico
                and rd.random() <= self.EXTEND_PROB_AT_SIZE[2]):
            ...
            if direction:
                ...
                print(f"Generando figura en L, en la figura {self.groups_count}")
                print (f"celda:{self.firts_cell.seq}, derecha:{self.firts_cell.next[RIGHT].seq}, derecha-derecha:{self.firts_cell.next[RIGHT].next[RIGHT].seq}")
                return True
    ...
    def _grow_figure(self) -> bool:
        open_directions: list = self._get_open_cells_directions(
            self.center_cell, self.last_direction, self.figure_size)
        ...
        if len(open_directions) == 0:
            ...
            # ! El -1 lo he puesto en un momento que no tengo control del programa, pero matemáticamente parece tener sentido
            direction = open_directions[rd.randint(
                0, len(open_directions) - 1)]
            self.last_direction = direction
        ...
        if (self.figure_size == 2
            ...
            for dir in range(4):
                if (self.center_cell.is_connected_at[dir]
                        and self._is_open_cell(self.firts_cell, dir, self.last_direction, self.figure_size)):
