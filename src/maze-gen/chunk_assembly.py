...
def generate_chunk_tilemap(
    chunk: Chunk, max_figure_size: int, max_attempts: int = 500
) -> np.ndarray:
    """
    ...
    for attempt in range(max_attempts):
        try:
            ...
            left_positions = (
                [y - chunk.start_row for y in chunk.left_tunnels]
                if chunk.left_tunnels
                else None
            )
            right_positions = (
                [y - chunk.start_row for y in chunk.right_tunnels]
                if chunk.right_tunnels
                else None
            )
            ...
            if not tunnels.is_valid_cell_map:
                logger.debug(
                    f"Attempt {attempt + 1}: Invalid tunnel configuration, retrying..."
                )
                continue
...
def assemble_chunks_vertical(chunk_tilemaps: List[np.ndarray]) -> np.ndarray:
    ...
    logger.debug(
        f"Assembled {len(chunk_tilemaps)} chunks vertically into shape {assembled.shape}"
    )
...
def assemble_layers_horizontal(layer_tilemaps: List[np.ndarray]) -> np.ndarray:
    ...
    logger.info(
        f"Assembled {len(layer_tilemaps)} layers horizontally into final maze shape {assembled.shape}"
    )
...
def generate_giant_maze_with_chunks(
    config: MazeConfig, max_macro_attempts: int = 1000000
) -> np.ndarray:
    """
    ...
    for macro_attempt in range(max_macro_attempts):
        try:
            ...
            if not is_navigable:
                logger.warning(
                    f"Macro attempt {macro_attempt + 1}: Chunk map not navigable, retrying..."
                )
                continue
            logger.info(
                f"Macro attempt {macro_attempt + 1}: Chunk map is navigable, generating chunks..."
            )
            ...
            for layer_id, chunks in enumerate(chunk_map):
                logger.info(
                    f"Processing layer {layer_id} with {len(chunks)} chunk(s)..."
                )
                chunk_tilemaps = []
                for chunk in chunks:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"  Expected tile size: ~{test_config.layer_rows * 3}×{test_config.num_layers * test_config.layer_cols * 3}"
        )
