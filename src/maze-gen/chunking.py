...
class Chunk:
    ...
    def __init__(
        self, layer_id: int, chunk_id: int, start_row: int, end_row: int, cols: int
    ):
        """Initialize a chunk."""
...
def chunk_layer_iterative(
    layer_id: int, total_rows: int, cols: int, min_chunk_size: int, max_chunks: int
) -> List[Chunk]:
    """
    ...
    if num_chunks == 1:
        chunk = Chunk(
            layer_id=layer_id, chunk_id=0, start_row=0, end_row=total_rows, cols=cols
        )
        return [chunk]
    ...
    for i in range(num_chunks):
        ...
        chunks.append(chunk)
        logger.debug(
            f"  Chunk {i}: rows {current_start}-{current_start + chunk_rows} (height={chunk_rows})"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Min chunk size: {test_config.min_chunk_size}, Max chunks: {test_config.max_chunks_per_layer}"
    )
    print(
        f"Probabilities: 1-chunk={Chunk.SINGLE_CHUNK_PROBABILITY}, 2-chunk={Chunk.TWO_CHUNKS_PROBABILITY}"
    )
    print()
