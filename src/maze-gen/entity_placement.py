...
def place_entities_on_tilemap(
    ...
    chomp_spawn = _find_chomp_spawn(modified_tilemap, config)
    ...
    metadata = {
        ...
        'chomp_spawn': tuple(int(x) for x in chomp_spawn) if chomp_spawn is not None else None,
...
def _place_powerpellets_spatially(
    ...
    """
    LOGGER.debug(f"Placing {num_powerpellets} power pellets with spatial distribution...")
    ...
    LOGGER.debug(f"Placed {actual_placed} power pellets across {grid_size}x{grid_size} regions")
    
    return modified_tilemap, actual_placed


def _find_chomp_spawn(
    tilemap: np.ndarray,
    config: MazeConfig
) -> Tuple[int, int] | None:
    """
    ...
    Strategy:
    - Look for the unique pacdot in the first column (x=0)
    ...
    spawn_pos = (int(pacdot_positions[spawn_idx][0]), int(pacdot_positions[spawn_idx][1]))
...
def _find_echo_spawns(
    tilemap: np.ndarray,
    num_echoes: int,
    chomp_spawn: Tuple[int, int] | None
) -> List[Tuple[int, int]]:
    """
    ...
    if len(echo_positions) < num_to_place:
        ...
        available = [(int(p[0]), int(p[1])) for p in candidate_positions if (int(p[0]), int(p[1])) not in placed_set]
    ...
    LOGGER.debug(f"Placed {len(final_positions)} echo spawns across {grid_size}x{grid_size} regions")
...
if __name__ == '__main__':
    ...
    print(f"Config: ghost_ratio={test_config.ghost_ratio}, powerpellet_ratio={test_config.powerpellet_ratio}\n")
