...
import { calculateRotation, calculateScaleFactor } from '@scenes/hooks/useEchoRenderState'
...
function EchoGroup({ mode, echos, geometry, materials }: EchosByMode & { geometry: THREE.BufferGeometry, materials: Map<BehaviorMode, THREE.Material> }) {
...
export default function Sinusoid({ echosMap }: InstancedSinusoidsProps) {
  const { nodes: baseNodes } = useGLTF('/assets/models/sinusoid-base.glb') as GLTFResult;
  const { nodes: frightenedNodes } = useGLTF('/assets/models/sinusoid-frightened.glb') as GLTFResult;
  const { nodes: chaseNodes } = useGLTF('/assets/models/sinusoid-chase.glb') as GLTFResult;
  ...
  const materials = useMemo(() => {
    ...
    // Frightened
    map.set(BehaviorMode.FRIGHTENED, frightenedNodes.mesh_0.material as THREE.Material);
